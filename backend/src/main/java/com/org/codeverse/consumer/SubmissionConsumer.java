...
import com.org.codeverse.repository.ScriptRepository;
...
//        System.out.println("Received submission " + submissionQueueDTO.getSubmission().getId());
        ...
        Script scriptObj = scriptRepository.findByLanguage(language)
                .orElseThrow(() -> new ResourceNotFoundException("Script", "language", language.toString()));

        String scriptFilename = RunQuestionUtil.generateScriptFile(scriptObj.getScript(), language);
...
//        String executableFilename = SubmitQuestionUtil.makeFile(snippet, driverCodes.get(0).getCode(), inputFilename);
        String executableFilename = RunQuestionUtil.generateExecutableFile(submissionId.toString(), code, ".java");
//        String response = SubmitQuestionUtil.runFile(executableFilename);
        ...
        try {
            String response = RunQuestionUtil.runFile(scriptFilename, executableFilename, inputFilename);
